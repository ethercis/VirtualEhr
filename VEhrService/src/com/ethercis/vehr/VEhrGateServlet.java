...
public class VEhrGateServlet extends HttpServlet implements
    ...
    public void init(String[] args) {
        ...
        if (controller == null) {
            log.error("AccessGateService (Active Enforcement Point) service is not loaded, please make sure it is defined in your services.xml file");
            throw new IllegalArgumentException(
                    "AccessGateService (Active Enforcement Point) service is not loaded, please make sure it is defined in your services.xml file");
        }
	...
	public void processRequest(MethodName action, HttpServletRequest servletRequest,
		...
		I_SessionClientProperties hdrprops = HttpParameters.getInstanceFromHeader(
                global, servletRequest);
		MethodName method;
	...
	public void syncExecute(MethodName action, I_SessionClientProperties header,String path, MethodName method, I_SessionClientProperties parameters,HttpServletResponse res) throws ServletException, IOException {
		...
		handleOutput(controller.getMappedMethodReturnType(action, path, method), output, res);
	}
...
//		enqueueTask(context, action, header, path, method, parameters);
		...
		executor.execute(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					if (response != null) {
						handleOutput(controller.getMappedMethodReturnType(action, path, method), output, response);
						ctx.complete();
					} else {
	 ...
	 * @throws java.io.IOException
	 */
	private void handleOutput(int returntype, Object output, Object res)
		...
		case MethodName.RETURN_JSON:
			new JsonHttpResponse(res).respond(output);
			break;
		case MethodName.RETURN_XML:
			new XmlHttpResponse(res).respond((String) output);
			break;
		...
		case MethodName.RETURN_DYNA:
			new DynamicHttpResponse(((ResponseHolder) output).getContentType(), res)
					.respond((ResponseHolder) output);
			break;

		default:
			new TextHttpResponse(res)
					.respond("undefined or unhandled return type for method");
			break;
		}
	 ...
	 * @throws ServletException
	 */
	public void errorOutput(HttpServletResponse response,
			ServiceManagerException exception) throws ServletException {
		response.setStatus(HttpServletResponse.SC_NO_CONTENT);
		int code = exception.getErrorCode().getHttpCode();

		// stuff in more details in the header...
		response.setHeader(I_SessionManager.ERROR_CODE,
				exception.getErrorCodeStr());
		response.setHeader(I_SessionManager.ERROR_MESSAGE,
