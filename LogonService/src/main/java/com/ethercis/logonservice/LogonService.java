...
public class LogonService extends ClusterInfo implements I_SessionManager {
    ...
    @QuerySetting(dialect = {
            ...
            @QuerySyntax(mode = DialectSpace.EHRSCAPE, httpMethod = "POST", method = "post", path = "rest/v1/session", responseType = ResponseType.Json)
            })
    ...
    @QuerySetting(dialect = {
            ...
            @QuerySyntax(mode = DialectSpace.EHRSCAPE, httpMethod = "DELETE", method = "delete", path = "rest/v1/session", responseType = ResponseType.Json)
            }
    )
	public Object disconnect(I_SessionClientProperties props)
			throws ServiceManagerException {
	...
	@Override
	public String getSubjectId(String secretSessionId) throws IllegalArgumentException{

		try {
			SessionInfo sessionInfo = check(secretSessionId);
			return sessionInfo.getSubjectInfo().getId();
		} catch (ServiceManagerException e) {
			throw new IllegalArgumentException("Invalid session id:"+secretSessionId);
		}
