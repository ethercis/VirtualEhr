   ...
   */
   ...
   private I_Service instantiateServiceFirstPhase(ServiceInfo ServiceInfo, boolean useServiceCache) 
      ...
      try {
         ...
         String text = "The Service class or initializer '" + ServiceName + "' is invalid, check the Service name, check if the Service has a default constructor and check the CLASSPATH to the Service";
         log.error(text);
         //e.printStackTrace();
         throw new ServiceManagerException(glob, SysErrorCode.RESOURCE_CONFIGURATION_SERVICEFAILED, ME+".Invalid", text, e);
      }
